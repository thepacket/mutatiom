...
describe("Lindblad open-system engine", () => {
  ...
  it("rate ratio reproduces Boltzmann between the lowest pair", () => {
    ...
    const rates = buildRates(sys, kT, BATH);
  ...
  it("conserves trace (Σ level populations = 1) throughout", () => {
    const traj = evolveLindblad(res, { tempK: 310, bath: BATH, samples: 60 });
  ...
  it("relaxes to the Boltzmann tautomer fraction (steady state)", () => {
    const traj = evolveLindblad(res, { tempK: 310, bath: BATH, samples: 200 });
  ...
  it("warmer bath drives a larger steady-state tautomer population", () => {
    const cold = evolveLindblad(res, { tempK: 150, bath: BATH, samples: 120 });
    const warm = evolveLindblad(res, { tempK: 350, bath: BATH, samples: 120 });
    expect(warm.steadyRight).toBeGreaterThan(cold.steadyRight);
  });

  it("reports a finite relaxation time", () => {
    const traj = evolveLindblad(res, { tempK: 310, bath: BATH, samples: 40 });
    expect(traj.relaxTimeFs).toBeGreaterThan(0);
    expect(Number.isFinite(traj.relaxTimeFs)).toBe(true);
  });

  it("relaxationTimeFs matches the evolved trajectory and scales with λ", () => {
    const direct = relaxationTimeFs(res, 310, BATH);
    const traj = evolveLindblad(res, { tempK: 310, bath: BATH, samples: 20 });
    expect(direct).toBeCloseTo(traj.relaxTimeFs, 3);
    // Stronger coupling (larger reorganization energy) ⇒ faster relaxation.
    expect(relaxationTimeFs(res, 310, bathWithReorg(0.6))).toBeLessThan(direct);
  });
});
